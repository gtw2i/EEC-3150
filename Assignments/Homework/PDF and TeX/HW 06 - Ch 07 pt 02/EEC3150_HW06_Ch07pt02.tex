...
This exercise will require you to write two \code{.py} files.
...
\begin{enumerate}[leftmargin=*]
	\item Write a program which will obtain a list of files in the cwd using \code{os.listdir()} and create a dictionary whose keys are filetypes and whose values are a list of files of that type (all files are taken from the list of files return by \code{os.listdir()}). For example, is the files \code{a.txt,b.txt,c.py} were the in directory, the program should create the following dictionary: \\
...
In this exercise, you will calculate the ages of several actors using the \code{re,time} modules. See the Birthdays.txt file. The dates are are formatted like mm/dd/yyyy.
\begin{enumerate}[leftmargin=*]
