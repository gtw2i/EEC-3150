...
\begin{frame}	
	...
	\begin{block}{Writing files in a \code{with,as} block:}
		...
		\minipage{0.5\textwidth}
			\begin{itemize}
				\item Open the file like before, but now with \code{`w'}
...
\begin{frame}	
	...
	\begin{exampleblock}{Write a 2D list to a file:}
		Create a 2D list of numbers and write its contents to a CSV (comma-separated value) file.
	\end{exampleblock}
	
	\begin{exampleblock}{Write electron density parameters from all files to a single file:}
		\begin{itemize}
			\item Write a function will accept a filename (any of these three: \code{h2o.txt, nh3.txt, ch4.txt}) and return lists of the \code{CENTRE ASSIGNMENTS} (note the spelling), \code{TYPE ASSIGNMENTS}, and \code{EXPONENTS}
			\item Write these all these lists to a single file
		\end{itemize}		
	\end{exampleblock}
...
\begin{frame}	
	...
	\begin{block}{Appending to files:}
		\minipage{0.5\textwidth}
			\code{fname = "Write.txt" \\
		...
		\minipage{0.5\textwidth}
			\begin{itemize}
				\item If you want to add more to an existing file without overwriting it, open the file in append mode (`a')
			\end{itemize}
		\endminipage\hfill
	\end{block}
	
	\begin{exampleblock}{Append electron density parameters from all files to a single file:}
		Modify the previous code to use append mode.
	\end{exampleblock}
